    ...
    public OrderResult processOrder(OrderRequest request) {
        ...
        try {
            ...
            for (MatchingEngine.Fill fill : matchResult.fills) {
                ...
                // Keep contra order state and outbound reports in sync with FIX-driven path.
                executionReportService.processContraFill(fill, null);
    ...
    private boolean isOrderTypeAllowedInPhase(String orderType) {
        ...
        if (phase == MarketStateManager.TradingPhase.OPENING_AUCTION ||
            phase == MarketStateManager.TradingPhase.CLOSING_AUCTION) {
             // Allow market orders in simulator/test flows as well so MiniFIX
             // scenarios can be matched without being rejected by session phase.
             return "LIMIT".equalsIgnoreCase(normalizedType) ||
                 "MARKET".equalsIgnoreCase(normalizedType);
        }
        if (phase == MarketStateManager.TradingPhase.PRE_OPEN ||
            phase == MarketStateManager.TradingPhase.POST_CLOSE) {
             // Extended sessions: allow passive orders and market orders for demo/test traffic.
            return "LIMIT".equalsIgnoreCase(normalizedType) ||
