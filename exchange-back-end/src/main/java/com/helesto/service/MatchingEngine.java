...
public class MatchingEngine {
     ...
     * If no liquidity exists, add to book at a reasonable market price
     */
    private void processMarketOrder(OrderBookManager.BookOrder order, MatchResult result) {
        ...
        if (matchableOrders.isEmpty()) {
            // No immediate liquidity - add market order to book at reference/market price
            // This allows the order to be matched when counterparty liquidity arrives
            double marketPrice = getCurrentMarketPrice(order.symbol, order.side);
            if (marketPrice <= 0) {
                // Fallback: use a default price if no reference price available
                marketPrice = order.price > 0 ? order.price : 100.0; // Default to 100 if no price given
            }
            
            // Set price for market order and add to book
            order.price = marketPrice;
            order.leavesQty = order.originalQty;
            orderBookManager.addOrder(order);
            
            result.status = OrderStatus.NEW;
            result.leavesQty = order.originalQty;
            result.filledQty = 0;
            result.avgPrice = 0;
            result.addedToBook = true;
            
            LOG.info("Market order {} added to book at price {} (no immediate liquidity)", 
                    order.orderId, marketPrice);
            return;
        }
        ...
        // Market order with partial fill - add remainder to book or reject based on TIF
        if (result.leavesQty > 0) {
            if (result.filledQty > 0) {
                // Partial fill - set remaining qty and add to book
                order.leavesQty = result.leavesQty;
                order.price = result.avgPrice > 0 ? result.avgPrice : getCurrentMarketPrice(order.symbol, order.side);
                orderBookManager.addOrder(order);
                result.status = OrderStatus.PARTIALLY_FILLED;
                result.addedToBook = true;
            } else {
                // No fill at all - add full quantity to book
                order.leavesQty = order.originalQty;
                order.price = getCurrentMarketPrice(order.symbol, order.side);
                if (order.price <= 0) {
                    order.price = 100.0; // Fallback price
                }
                orderBookManager.addOrder(order);
                result.status = OrderStatus.NEW;
                result.leavesQty = order.originalQty;
                result.addedToBook = true;
                LOG.info("Market order {} added to book - no matching counterparty found", order.orderId);
            }
        } else {
            // Fully filled
