...
public class FixOrderManagementEngine {
    ...
    public void processOrderMatch(OrderEntity order, SessionID sessionID) {
        try {
            ...
            // Handle terminal outcomes with no fills (e.g., FOK/IOC/market no-liquidity)
            // so broker side does not remain stuck in NEW/PENDING_NEW.
            if (result.filledQty == 0 && !result.addedToBook) {
                order.setUpdatedAt(LocalDateTime.now());

                if (result.status == MatchingEngine.OrderStatus.CANCELED) {
                ...
                if (result.status == MatchingEngine.OrderStatus.REJECTED) {
                    String reason = result.rejectReason != null && !result.rejectReason.isBlank()
                            ? result.rejectReason
                            : "Order rejected by matching engine";
                    order.setStatus("REJECTED");
                    order.setLeavesQty(0L);
                    orderDao.updateOrder(order);
                    executionReportService.sendReject(order, OrdRejReason.OTHER, reason, sessionID);
                    return;
                }
            }
        } catch (Exception e) {
