...
public class OrderManagementRest {
    ...
    private Response processSingleOrder(OrderSubmitRequest request) {
        ...
        waitForLiquidityVisibility(request);
    ...
    private void waitForLiquidityVisibility(OrderSubmitRequest request) {
        String orderType = convertOrderType(request.orderType != null ? request.orderType : "LIMIT");
        if (!"LIMIT".equals(orderType) && !"STOP_LIMIT".equals(orderType)) {
            return;
        }

        int configuredWaitMs;
        if (request.matchWaitMs != null) {
            configuredWaitMs = request.matchWaitMs;
        } else if (request.clientId != null && "EXCHANGE_UI".equalsIgnoreCase(request.clientId)) {
            configuredWaitMs = 750;
        } else {
            configuredWaitMs = 0;
        }

        int waitMs = Math.max(0, Math.min(3000, configuredWaitMs));
        if (waitMs == 0) {
            return;
        }

        String normalizedSide = request.side;
        if ("BUY".equalsIgnoreCase(normalizedSide)) {
            normalizedSide = "1";
        } else if ("SELL".equalsIgnoreCase(normalizedSide)) {
            normalizedSide = "2";
        }

        long deadline = System.currentTimeMillis() + waitMs;
        while (System.currentTimeMillis() < deadline) {
            List<OrderBookManager.BookOrder> matchable = orderBookManager
                    .getMatchableOrders(request.symbol, normalizedSide, request.price);
    ...
    public static class OrderSubmitRequest {
        ...
        public Integer matchWaitMs; // Optional race guard window for resting liquidity visibility
    }
