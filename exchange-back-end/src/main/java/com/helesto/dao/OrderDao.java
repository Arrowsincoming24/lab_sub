...
public class OrderDao {
    ...
    @Transactional
    public List<OrderEntity> findActiveOrdersForOrderBookRecovery() {
        LOG.debug("Finding active orders for order book recovery");
        return em.createQuery(
                "SELECT o FROM OrderEntity o WHERE o.status IN ('NEW', 'PARTIALLY_FILLED') " +
                "AND o.leavesQty > 0 ORDER BY o.createdAt ASC",
                OrderEntity.class)
                .getResultList();
    }
