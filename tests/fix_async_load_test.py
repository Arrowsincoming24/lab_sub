...
def resolve_sender_id(user_id: int, config: TestConfig) -> str:
    if config.sender_mode == "exchange":
        allowed = ["MINIFIX", "MINIFIXJ", "MINIFIX-J"]
        return allowed[user_id % len(allowed)]
    return f"USER_{user_id:04d}"


async def simulate_user(user_id: int, config: TestConfig, gate: asyncio.Semaphore) -> UserResult:
    sender_id = resolve_sender_id(user_id, config)
    ...
    async with gate:
        ...
        while attempt <= config.max_session_retries:
            ...
            try:
                ...
                for side in ("1", "2"):
                    acked = False
                    for order_attempt in range(2):
                        cl_ord_id = f"CLID_{sender_id}_{side}_{int(time.time() * 1000)}_{order_attempt}"
                        ...
                        try:
                            ...
                            result.latency_issues += 1
                            result.errors.append(f"Order ack timeout side={side} attempt={order_attempt + 1}")

                    if not acked:
                        result.errors.append(f"Order flow failed side={side} after retries")
...
def parse_args() -> TestConfig:
    ...
    parser.add_argument("--sender-mode", choices=["user", "exchange"], default="user")
